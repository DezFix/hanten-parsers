...
package hanten.wre.app.parsers.site.madara.en.theblank;
...
public class SecretStream {
  ...
  public PullResult pull(State state, byte[] in, int inlen, byte[] ad, int adlen) {
    ...
    ChaCha20.streamIETF(block, 64, state.nonce, state.k);
    {
      ...
      for (int i = 0; i < 32; i++) sb.append(String.format("%02x", block[i]));
      System.out.println("Java poly_key:    " + sb.toString());
    }
    ...
    if (!constantTimeCompare(mac, storedMac)) {
      ...
      for (byte b : storedMac) expected.append(String.format("%02x", b));
      for (byte b : mac) actual.append(String.format("%02x", b));
      Arrays.fill(mac, (byte) 0);
      throw new RuntimeException("MAC mismatch: expected " + expected.toString() + " got " + actual.toString() + " mlen=" + mlen);
    }
    ...
    // Update nonce with MAC (CRITICAL FIX)
    for (int i = 0; i < 8; i++) {
